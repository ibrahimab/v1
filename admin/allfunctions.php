...
if (!function_exists("datum")) {
	function datum($a,$time=0,$language='nl') {
		...
		if($language=="nl") {
			...
			$a = ereg_replace("(^|[^a-zA-Z0-9‰])DD MND","\\1DD. MND",$a);
			$a = ereg_replace("(^|[^a-zA-Z0-9‰])D MND","\\1D. MND",$a);
			$a = ereg_replace("(^|[^a-zA-Z0-9‰])DD MAAND","\\1DD. MAAND",$a);
			$a = ereg_replace("(^|[^a-zA-Z0-9‰])D MAAND","\\1D. MAAND",$a);
