...
#
#
# Dit script wordt op elke dag om 12.30u. gerund op de server web01.chalet.nl met het account chalet01.
#
# /usr/bin/php /var/www/chalet.nl/html/cron/elkedag.php test
#
...
if ($vars["lokale_testserver"]) {
	$db->query("SELECT b.boekingsnummer, b.mailblokkeren_opties, b.boeking_id, b.aankomstdatum, b.aankomstdatum_exact, b.website, b.mailtekst_opties, a.naam, a.mailtekst_id, t.type_id, t.naam AS tnaam, p.naam AS plaats, l.begincode FROM boeking b, accommodatie a, type t, plaats p, land l, skigebied s WHERE b.type_id=t.type_id AND t.accommodatie_id=a.accommodatie_id AND a.plaats_id=p.plaats_id AND p.skigebied_id=s.skigebied_id AND p.land_id=l.land_id AND b.factuurdatum IS NOT NULL AND b.geannuleerd=0 AND b.stap_voltooid=5 AND b.goedgekeurd=1 AND b.mailverstuurd_opties IS NULL AND b.aankomstdatum>='".mktime(0,0,0,date("m"),date("d")+1,date("Y"))."' AND a.wzt=2 AND b.mailblokkeren_opties=0".($reisbureau_user_blokkeren ? " AND (b.reisbureau_user_id IS NULL OR b.reisbureau_user_id NOT IN (".$reisbureau_user_blokkeren."))" : "")." ORDER BY b.aankomstdatum_exact DESC, s.naam, p.naam, b.boekingsnummer LIMIT 0,1;");
} else {
