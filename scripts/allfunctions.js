...
var wt_removeDiacritics_changes;
function wt_removeDiacritics (str) {
	if(!wt_removeDiacritics_changes) {
		wt_removeDiacritics_changes = defaultDiacriticsRemovalMap;
	}
	for(var i=0; i<wt_removeDiacritics_changes.length; i++) {
		str = str.replace(wt_removeDiacritics_changes[i].letters, wt_removeDiacritics_changes[i].base);
	}
...
$(document).ready(function() {
	...
	$(".wtform_multiradio_check_all").click(function() {
		...
		$(".wtform_multiradio_fields_"+wtform_multiradio_id).each(function() {
			...
			if($(this).attr("value")==wtform_multiradio_value) {
