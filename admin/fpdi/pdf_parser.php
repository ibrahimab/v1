<?php
// +---------------------------------------------------------------------+
// | FPDI PDF-Parser v.1.0.4                                             |
// | Copyright (c) 2009-2012 Setasign - Jan Slabon                       |
// +---------------------------------------------------------------------+
...
class pdf_parser {
            ...
            if (is_array($xrefStreamObjDec) && isset($xrefStreamObjDec[0]) && $xrefStreamObjDec[0] == PDF_TYPE_OBJDEC) {
                ...
                if (isset($xrefStream[1][1]['/Type']) && $xrefStream[1][1]['/Type'][1] == '/XRef') {
                    ...
                    foreach ($filters AS $_filter) {
                        switch ($_filter[1]) {
                            case '/FlateDecode':
                            	$oXrefStreamData = $xrefStreamData;
                                ...
                                if ($xrefStreamData === false) {
                                	$xrefStreamData = @(gzinflate(substr($oXrefStreamData, 2)));
                    ...
                    if (count($filters) > 0 && isset($xrefStream[1][1]['/DecodeParms']) && isset($xrefStream[1][1]['/DecodeParms'][1]['/Predictor'])) {
    ...
    function pdf_resolve_object(&$c, $obj_spec, $encapsulate = true) {
    	...
    	if ($obj_spec[0] == PDF_TYPE_OBJREF) {
    		...
    		} elseif (isset($this->xref['objStreams'][$obj_spec[1]])) {
                    ...
                    foreach ($filters AS $_filter) {
                        switch ($_filter[1]) {
                            case '/FlateDecode':
                                ...
                                if ($stream === false) {
                                	$stream = @(gzinflate(substr($oStream, 2)));
