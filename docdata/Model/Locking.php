...
class Model_Locking extends Model {
    ...
    public function __construct($lock_code) {
        ...
        if (empty($lock_code)) {
			$_helper->log(get_class() . ': Empty lock code given', App::WARN);
            return false;
        }
		...
		if (!$this->num_rows()) {
            $this->_helper->log(get_class() . ': Lock table "'.$this->_lockTable.'" does not exist. Locking is not possible. ' . $this->_lockInfo, App::ERR);
            return false;
        }
    ...
    public function lock() {
        ...
        if ($result) {
            $this->_locked = true;
            $this->_helper->log(get_class() . ': Lock acquired. ' . $this->_lockInfo, App::DEBUG);
        } else {
            $this->_helper->log(get_class() . ': Lock acquire failed. ' . $this->_lockInfo, App::WARN);
        }
    ...
    private function _insertLock() {
        ...
        $nowTime = time();
        $this->_helper->log(get_class() . ': Trying to get lock by insert. ' . $this->_lockInfo, App::DEBUG);
    ...
    private function _updateLock() {
        ...
        $this->_helper->log(get_class() . ': Trying to get lock by update. ' . $this->_lockInfo, App::DEBUG);
    ...
    public function unlock() {
        ...
        $this->_helper->log(get_class() . ': Trying to free lock. ' . $this->_lockInfo, App::DEBUG);
        ...
        if ($result) {
            $this->_locked = false;
            $this->_helper->log(get_class() . ': Lock freed. ' . $this->_lockInfo, App::DEBUG);
        } else {
            $this->_helper->log(get_class() . ': Freeing lock failed. ' . $this->_lockInfo, App::WARN);
        }
    ...
    private function _maintenance() {
        ...
        $this->_helper->log(get_class() . ': Performing maintenance, removing locks older than '.$cleanupTime, App::DEBUG);
        ...
        $this->_helper->log(get_class() . ': Maintenance performed, ' . $result . ' rows removed.', App::DEBUG);
        return $result;
    }
    ...
    public function shutdown() {
        ...
        if ($this->_locked) {
            $this->_helper->log(get_class() . ': Still locked on shutdown, so unlocking. ' . $this->_lockInfo, App::WARN);
            $this->unlock();
        }
    }
    ...
    public function initCheck() {
        if (!$this->_initOK) {
            $this->_helper->log(get_class() . ': Object is not initialed correctly, no locking possible. ' . $this->_lockInfo, App::ERR);
        }
        return $this->_initOK;
    }
