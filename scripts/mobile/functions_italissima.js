...
function updateMapSize(){
        height = jQuery(".wrapper").width()* 1.204;
        jQuery("#vmap").css("width", jQuery(".wrapper").width());
        jQuery("#vmap").css("height", height);
        pin_width = jQuery(".wrapper").width()/18.84;
        pin_height = pin_width * 1.73;
        pin_left = jQuery(".wrapper").width() / 3.14;
        pin_top = height / 7.46;
        jQuery(".pin_content").width(pin_width);
        jQuery(".pin_content").height(pin_height);
        jQuery(".jqvmap_pin").css("left",pin_left);
        jQuery(".jqvmap_pin").css("top", pin_top); 
}
...
$(document).ready(function() {
        ...
        $(".zoekblok_aankomstdatum").addClass("chzn-container-single");
        $("#zoekenboekdate").datepicker({
            showOn: "button",
            buttonImage: "/pic/class.form_calendar.gif",
            buttonImageOnly: true,
            onSelect: function(dateText, inst){
                selected_day = parseInt(inst.selectedDay) + 1;
                selected_month = parseInt(inst.selectedMonth) +1;
                $('select[name=fadf_d]')
                    .find('option:nth-child('+selected_day+')').prop('selected', true)
                    .end().trigger('liszt:updated');
            
                $('select[name=fadf_m]')
                    .find('option[value='+selected_month+']').prop('selected', true)
                    .end().trigger('liszt:updated');
                $('select[name=fadf_y]')
                    .find('option[value='+inst.selectedYear+']').prop('selected', true)
                    .end().trigger('liszt:updated');
                $("#zoeken").submit();
            }
        });
        
        $("#toonaccommodatiedate").datepicker({
            showOn: "button",
            buttonImage: "/pic/class.form_calendar.gif",
            buttonImageOnly: true,
            onSelect: function(dateText, inst){
                ...
                accommodatiepagina_flexibel_onchange();
                //$("#zoeken").submit();
            }
        });
        
        $("#boekendate").datepicker({
            showOn: "button",
            buttonImage: "/pic/class.form_calendar.gif",
            buttonImageOnly: true,
            onSelect: function(dateText, inst){
                selected_day = parseInt(inst.selectedDay) + 1;
                selected_month = parseInt(inst.selectedMonth) +1;
                $('select[name="input[aankomstdatum_flex][day]"]')
                    .find('option:nth-child('+selected_day+')').prop('selected', true)
                    .end().trigger('liszt:updated');
            
                $('select[name="input[aankomstdatum_flex][month]"]')
                    .find('option[value='+selected_month+']').prop('selected', true)
                    .end().trigger('liszt:updated');
                
                $('select[name="input[aankomstdatum_flex][year]"]')
                    .find('option[value='+inst.selectedYear+']').prop('selected', true)
                    .end().trigger('liszt:updated');
            }
        });
