...
#	var $sousdir="temp/";	// sous repertoire dans lequel s'effectue la sauvegarde avec le / final
	var $sousdir="/tmp/";	// sous repertoire dans lequel s'effectue la sauvegarde avec le / final
...
//************** dump de la base ***************** beging the dump****
	...
	{		
			...
			while ($i < $nb_row) {
				...
				if($_GET["ignorefast"]) {
					unset($ignore_table);
					$ignorefast=split(",",$_GET["ignorefast"]);
					@reset($ignorefast);
					while(list($key,$value)=@each($ignorefast)) {
						if(ereg("_WILDCARD_",$value)) {
							$value=ereg_replace("_WILDCARD_",".*",$value);
							if(ereg($value,$tablename)) {
								$ignore_table=true;
							}
						} elseif($tablename==$value) {
							$ignore_table=true;
						}
					}
				}
...
#					$this->ecrire("$create\n\n");
				...
				if($this->data_yes==1) {
					...
					if($this->maximaal_aantal_rows and !$this->unbuffered_query) {
						...
						unset($maximaal_aantal_rows);
					}
					unset($met_loop,$loop_max,$loop_teller);
					if($maximaal_aantal_rows and mysql_num_rows($datacreate) > $maximaal_aantal_rows) {
						$met_loop=true;
						$loop_teller=ceil(mysql_num_rows($datacreate)/$maximaal_aantal_rows);
					} else {
						$met_loop=false;
						$loop_teller=0;
					}
					for($loop_max=0;$loop_max<=$loop_teller;$loop_max++) {
						...
						if($met_loop) {
							...
							$datacreate = mysql_query($newquery);
						} else {
							# query niet opnieuw
						}
						...
						while($row12 = mysql_fetch_assoc($datacreate)) {
							$rowteller++;
							if($this->unbuffered_query and !$header_getoond) {
								$this->backup_comment("debut_dump", $tablename);
								// sauvegarde des donnees
								$qinsert = "LOCK TABLES `$tablename` WRITE; \n";
								$qinsert .= "INSERT INTO `$tablename` VALUES \n  ";
...
#							   if($this->no_time_limit){set_time_limit(30);}
							...
							unset($newrow);
							while(list($key,$value)=@each($row12)) {
								if(is_null($value)) {
									$newrow[]="NULL";
								} else {
									$newrow[]="'".mysql_escape_string($value)."'";
								}
							}
#							   $row12 = array_map(array($this, 'separe'), $row12);	// mise en forme des data dans le tableau
		...
		$this->ecrire("-- einde phpmysqldump: ".date("r")." (".round((time()-$this->starttime)/60)." minutes)\n");
