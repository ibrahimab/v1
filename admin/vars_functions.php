...
#		$return["stap1"]["factuur_tekstvak5"]=$db->f("factuur_tekstvak5");
		...
		$return["stap1"]["geannuleerd_op"]=$db->f("geannuleerd_op");
		...
		$return["stap1"]["roominglist_show_all_names"]=$db->f("roominglist_show_all_names");
...
function volledigeaccnaam($typeid) {
	...
	if(!$vars["volledigeaccnaam"]) {
		$db->query("SELECT a.naam AS accommodatie, a.soortaccommodatie, t.type_id, t.naam".$vars["ttv"]." AS type, t.code, t.optimaalaantalpersonen, t.maxaantalpersonen, p.naam".$vars["ttv"]." AS plaats, l.naam AS land, l.begincode FROM type t, accommodatie a, plaats p, land l WHERE t.accommodatie_id=a.accommodatie_id AND a.plaats_id=p.plaats_id AND p.land_id=l.land_id ORDER BY t.type_id;");
		while($db->next_record()) {
			unset($naam);
...
function googleanalytics() {
	...
	if($vars["googleanalytics"]) {
		if($test_analytics or (!$voorkant_cms and !in_array($_SERVER["REMOTE_ADDR"], $vertrouwde_ips) and !$_GET["wtfatalerror"])) {
